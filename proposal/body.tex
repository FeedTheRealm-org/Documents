...
This project consists of developing a software product in the form of an \textbf{MMO RPG Maker} game,
...
Multiplayer, online, content creation, concurrency, MMO.
...
ofreciendo herramientas para que los usuarios no solo puedan desarrollar sus mundos y aventuras, sino también promocionarlos y difundirlos dentro de la plataforma.
...
A nivel tecnológico, el avance de infraestructuras en la nube, como AWS GameLift, Microsoft PlayFab o Photon, ha permitido que los desarrolladores 
...
de manera que cualquier usuario pueda crear y modificar sin barreras técnicas.
...
esta propuesta requiere instanciar múltiples mundos de manera simultánea, cada uno con baja latencia y alto rendimiento para garantizar una experiencia de juego fluida.
\vspace{0.25cm}

Además, se deben considerar aspectos clave como:

\begin{itemize}
    \item Escalabilidad: ¿Cómo gestionar de manera eficiente cientos de clientes conectándose a un mismo mundo, y miles distribuidos en diferentes instancias?
    \item Baja latencia: Garantizar tiempos de respuesta mínimos en un entorno de interacción en tiempo real.
    \item Costos de infraestructura: ¿Qué inversión implica desplegar y mantener un sistema de estas características, considerando servidores,
    balanceadores, bases de datos distribuidas y servicios en la nube?
    \item Orquestación de instancias de mundos: Se requiere un sistema dinámico de autoescalado que levante o apague instancias según la demanda para evitar desperdicio de recursos.
    \item Balanceo de carga y distribución geográfica: La baja latencia demanda servidores distribuidos regionalmente mediante CDNs, edge computing o clusters dedicados.
    \item Persistencia de datos en tiempo real: Es necesario guardar progresos e inventarios en bases de datos distribuidas equilibrando consistencia y escalabilidad.
    \item Tolerancia a fallos y continuidad del servicio: La arquitectura debe haber fault tolerance con replicación y recuperación automática ante caídas de servidores.
    \item Seguridad y prevención de abusos: El sistema debe mitigar ataques de cheating, botting o DDoS mediante validación de tráfico y monitoreo continuo.
    \item Costos operativos: Los principales gastos estarán en servidores de baja latencia y almacenamiento persistente, requiriendo una planificación eficiente.
    \item Estrategias de monetización ligadas a la infraestructura: El modelo elegido (suscripción, micropagos o marketplace) determinará la asignación de recursos 
...
por el cual podremos mantener durante mucho tiempo funcionando el proyecto, es la base de sustentación operativa elemental del proyecto.
\vspace{0.25cm}

La cuestión a enfrentarse implica como cobrar dentro del proyecto, como así también que porcentajes.
Cómo impacta cada una de las decisiones de cobro, como el contexto afecta a cada una de las políticas, como reaccionan cada uno de los agentes ante cambios en las políticas de cobro.
\vspace{0.25cm}
...
mientras que los personajes y elementos animados se desarrollarán en 2D. Esta elección facilita tanto el diseño por parte del equipo como la creación de skins por los usuarios,
...
y accesible para la comunidad de creadores.

\subsection{Creación de mundos por usuarios}
El creador de mundos será un servicio que los usuarios podrán instalar para diseñar y personalizar mapas.
Al iniciar un nuevo proyecto, se definirá el tamaño del mapa (pequeño, mediano o grande) y, opcionalmente, se podrá importar un template preexistente como punto de partida.
\vspace{0.25cm}

Dentro de cada mundo existirán tres tipos principales de entidades:

\begin{itemize}
    \item \underline{Assets}: objetos colocables directamente en el mapa.
...
utilizar la librerías de entidades precargadas en la herramienta. Los mundos tendrán una estructura plana y cuadrada en entornos 3D,
con dimensiones proporcionales al tamaño de un jugador. Si un jugador mide una unidad, un mapa considerado “pequeño” deberá tener un mínimo de 500 unidades de extensión,
escalando dicho valor para los tamaños superiores.
\vspace{0.25cm}

Los usuarios podrán importar modelos 3D y asignarles comportamientos mediante interfaces abstractas. Por ejemplo, un modelo de criatura podrá configurarse como “Enemigo”,
...
Asimismo, los creadores tendrán la posibilidad de diseñar \textbf{quests}, que incentiven la exploración y la interacción con el mundo. De forma abstracta,
un quest podrá dispararse mediante un trigger asociado a una acción, como recoger un ítem o interactuar con un NPC. Al completarlo,
los jugadores recibirán recompensas definidas por el creador, tales como ítems o cosméticos.
\vspace{0.25cm}
...
Por último, está la implementación de tiendas virtuales para cada mundo que se detalla en mayor profundidad en la sección de \textbf{Mantenimiento y Monetización}.

\subsection{Arquitectura e Infraestructura}
La arquitectura de este proyecto está diseñada, por una parte, servidores de zona escalables, con el objetivo de soportar múltiples mundos simultáneos
y garantizar baja latencia para los jugadores. 
\vspace{0.25cm}

Por otra parte, un modelo de servidor con múltiples funcionalidades de estilo monolítico, donde estarán presentes distintos sistemas, como el sistema de cobranzas,
el manejo de datos de los mundos, el sistema del portal principal de mundos, etc.
Esto se desarrollará bajo ese tipo de arquitectura debido a que la mayor dificultad del proyecto se encuentra en que la otra parte mencionada 
llevará una arquitectura más compleja y que deberá optimizarse mucho para su correcto funcionamiento, además de que si hubiéramos optado 
por un sistema de microservicios esto llevaría más tiempo y posibles complicaciones, que no son la característica principal del desafío planteado, 
y se consideró al sistema elegido suficiente para el alcance del proyecto, aunque se espera crear un sistema bien elaborado y escalable.
\vspace{0.25cm}

Los usuarios cuentan con dos ejecutables principales: el cliente de juego, que les permite navegar y conectarse a distintos mundos a jugar, y el World Maker,
una aplicación destinada a los creadores para diseñar y publicar mapas. 
Ambos se comunican con la plataforma a través de un Gateway central que actúa como punto de entrada único, manejando la seguridad y el ruteo de todas las peticiones.
\vspace{0.25cm}
...
ya que permite que cada microservicio y servidor de zona se conecte de forma desacoplada, habilitando así la escalabilidad independiente de cada parte del sistema.
Gracias a este diseño, si aumenta la cantidad de jugadores, se pueden levantar más instancias de los servicios que lo necesiten sin afectar al resto.
\vspace{0.25cm}
...
y tolerante a fallos, mientras que el modelo de microservicios otorga modularidad y capacidad de crecimiento. 
...
\begin{itemize}
    \item \textbf{Gemas:} moneda premium adquirida con dinero real, utilizable en cualquier mundo para obtener ítems y cosméticos.
    Este sistema está orientado a incentivar a los creadores a diseñar y ofrecer contenido estético atractivo.
    ...
    combates o eventos, y se utiliza para adquirir ítems dentro de ese entorno particular.
\end{itemize}
...
Además, se implementará un sistema de suscripción para creadores de mundos, que será necesario para mantener el mundo funcionando online.
Inicialmente, se ofrecerá un período de prueba gratuito de dos semanas; posteriormente, el costo de mantenimiento dependerá del tamaño y la demanda del mundo.
\vspace{0.25cm}

El objetivo es que los mundos estén bien diseñados, que incentiven la interacción y el consumo de ítems, puedan sostener sus propios costos gracias al gasto de los jugadores,
...
El principal desafío técnico del proyecto, como se establece en la introducción, es la construcción de una infraestructura escalable y robusta. Para validar este aspecto de manera cuantitativa, se ejecutarán conjuntos de pruebas de rendimiento y estrés centradas en los servidores de zona (Clase A, B y C). El objetivo es medir la capacidad del sistema para sostener la conexión simultánea de cientos de jugadores en una misma instancia, procesar miles de transacciones en tiempo real y garantizar la estabilidad bajo un alto volumen de conexiones concurrentes. Además nos servirá para determinar la carga máxima al momento que se haga la prueba en la etapa final.
