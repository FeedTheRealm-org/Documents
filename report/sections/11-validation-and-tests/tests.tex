...
Como se menciono en la seccion de \textit{Devops} y \textit{CI/CD}, se cuenta con un sistema de pruebas unitarias y de 
aceptación, para garantizar la calidad del código, la funcionalidades el correcto funcionamiento del los servicio. 
Utilizando \textbf{Go} para las pruebas unitarias, y \textbf{Behave} con \textbf{Python} para las pruebas de aceptación.
\vspace{0.25cm}

Las pruebas unitarias, se encargan de probar por cada uno de los módulos del \textit{core-service}, las carpetas
\textit{services} para los servicios, \textit{repositories} para los repositorios. A su veces, se realizaron pruebas
unitarias para \textit{utils} y \textit{middleware}.
\vspace{0.25cm}
...
resultado un porcentaje de cobertura del 69.69\%, lo que garantiza una buena calidad del código.
\vspace{0.25cm}
...
que simulan el comportamiento de los usuarios, y así garantizar que el servicio se comporte de la manera esperada, sin 
importar el comportamiento de los usuarios. Para esto se utilizo, como se menciona, la herramienta \textbf{Behave} con 
\textbf{Python}.
\vspace{0.25cm}
...
es de 20, lo que garantiza una buena cobertura de los requisitos funcionales del servicio.
\vspace{0.25cm}
