...
\textit{Assemblies} y \texttt{Vcontainer}, esto se explica en la sección 
...
Estos elementos se inyectan con \texttt{VContainer}, lo que mejora la gestión de dependencias, 
la modularidad y elimina la necesidad de usar \texttt{Unity} para asignar componentes. 
Así el código queda desacoplado: las clases no conocen a otras ni a quién llegan sus eventos, 
compartiendo solo la lógica necesaria. Esto mantiene el código limpio y evita que cambios en un 
componente afecten a otros.
...
del cliente y toda la configuración necesaria para que la escena tome lo necesario para comenzar a funcionar y
...
en el contenedor de dependencias de \texttt{VContainer}, bajo clases inicializadoras de cada escena.
...
de moneda de juego, etc. Y luego otra escena para el juego una vez dentro del mundo, donde se desarrolla la 
...
Un componente importante dentro del concepto \textit{liking}, es el \textit{network adapter}. Éste actúa como 
