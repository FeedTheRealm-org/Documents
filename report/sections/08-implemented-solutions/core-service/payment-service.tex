...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.75\textwidth]{img/08-implemented-solution/core-service/payment-service-case-1.png}
...
se gestiona por \textit{slots}, donde cada \textit{slot} permite a un creador activar un mundo. El estado de la suscripción 
activa del usuario puede consultarse, en cualquier momento, mediante \texttt{GET /subscriptions/status}.
\vspace{0.25cm}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.75\textwidth]{img/08-implemented-solution/core-service/payment-service-case-2.png}
...
\texttt{PUT /subscriptions/slots}, y cancelarla mediante \texttt{DELETE /subscriptions}, siempre que no tenga
\textit{slots} en uso en ese momento. Los eventos que se reciben, respectivamente, son \texttt{customer.subscription.updated}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.95\textwidth]{img/08-implemented-solution/core-service/payment-service-case-3.png}
...
a dólares mediante la tasa de cambio configurada (\texttt{DollarsGems} \texttt{Ratio}). Adicionalmente, se actualizan
las métricas globales de gemas gastadas.
\vspace{0.25cm}

Cada vez que un usuario adquiere un cosmético creado por otro usuario, el sistema acredita automáticamente una
porción del valor de la venta al creador en su balance, expresado en dólares. Este balance se acumula a lo largo
del tiempo con cada compra que reciba alguno de sus cosméticos.
\vspace{0.25cm}

Un creador puede consultar su propio balance en cualquier momento mediante \texttt{GET /payments} \texttt{/balances/creators},
que retorna el monto acumulado asociado al usuario autenticado. Los administradores disponen adicionalmente del
