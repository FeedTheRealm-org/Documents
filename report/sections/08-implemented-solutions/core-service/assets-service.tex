...
\begin{itemize}
    \item \underline{\textit{Cosmetics}}: Corresponden a los \textit{sprites} que los jugadores pueden comprar para
    utilizarlos en sus personajes a través de los \textit{shops}.
...
nulo la operación queda restringida a administradores; de lo contrario, se verifica la propiedad del mundo antes
...
tenga privilegios de administrador; de lo contrario, la petición es rechazada. Los resultados se paginan mediante
los parámetros \texttt{offset} y \texttt{limit}, con valores por defecto de \texttt{0} y \texttt{24}
respectivamente y un tope máximo de \texttt{200} elementos por página. La respuesta incluye el campo
\texttt{total\_count} con el total de cosméticos disponibles para la consulta realizada, lo que permite al cliente
...
\hline
\caption{\textit{Endpoints} del \textit{assets service}}
\label{tab:assets-endpoints}
\end{longtable}
